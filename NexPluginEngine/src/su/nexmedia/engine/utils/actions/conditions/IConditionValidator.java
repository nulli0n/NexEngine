...
public abstract class IConditionValidator extends Parametized {
    ...
    @Nullable
    protected abstract Predicate<Entity> validate(@NotNull Entity exe, @NotNull Set<Entity> targets,
            @NotNull IParamResult result);

    public final boolean process(@NotNull Entity exe, @NotNull Map<String, Set<Entity>> targetMap,
            @NotNull String fullStr) {
        return this.process(exe, targetMap, fullStr, null);
    }

    public final boolean process(@NotNull Entity exe, @NotNull Map<String, Set<Entity>> targetMap,
            @NotNull String fullStr, @Nullable ActionManipulator manipulator) {
