...
public abstract class IAbstractCommand<P extends NexPlugin<P>> {
    ...
    @NotNull
    public List<@NotNull String> getTab(@NotNull Player player, int i, @NotNull String @NotNull [] args) {
        return Collections.emptyList();
    }

    protected abstract void perform(@NotNull CommandSender sender, @NotNull String label,
    ...
    protected final void printUsage(@NotNull CommandSender sender) {
        ...
        plugin.lang().Core_Command_Usage.replace("%usage%", usage()).replace("%cmd%", subLabel).replace("  ", " ")
                .replace("%label%", mainLabel).send(sender);
    }
    ...
    protected final void errType(@NotNull CommandSender sender, @NotNull Class<?> clazz) {
        plugin.lang().Error_Type.replace("%types%", CollectionsUT.getEnums(clazz)).send(sender);
    }
